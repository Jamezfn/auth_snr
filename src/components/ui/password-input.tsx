...
// Cache the factory instance once options are loaded, so we don't
// rebuild the dictionaries/graphs on every keystroke.
...
export function PasswordInputStrengthChecker() {
  ...
  const strengthResult = useMemo((): ZxcvbnResult | { score: 0; feedback: { warning: undefined } } => {
    if (!optionsLoaded || deferredPassword.length === 0 || zxcvbnInstance == null) {
      return { score: 0, feedback: { warning: undefined } } as const
    }

    return zxcvbnInstance.check(deferredPassword)
